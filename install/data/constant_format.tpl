...
// Unique identifier of the TravianZ world generated by this configuration.
// The installer creates one complete config.php per instance under /instances/.
define("INSTANCE_ID", "%INSTANCEID%");

// Multi-instance public-world metadata. These values are generated during
// installation and are consumed by the shared homepage/instance registry.
define("SERVER_NUMBER", %SERVERNUMBER%);
define("SERVER_CLASSIC", %SERVERCLASSIC%);
define("SERVER_SPEED", %SERVERSPEED%);
define("SERVER_ENABLED", %SERVERENABLED%);

// Runtime files are isolated per TravianZ world. The installer creates this
// directory before generating the configuration, so cron locks, cron markers,
// temporary state and future per-instance runtime files never collide with
// another world using the same codebase.
define("INSTANCE_RUNTIME_PATH", __DIR__ . DIRECTORY_SEPARATOR . "runtime" . DIRECTORY_SEPARATOR);
...
define("LANG", ($__user_lang !== '' && is_file(dirname(__DIR__, 2) . "/GameEngine/Lang/" . $__user_lang . ".php")) ? $__user_lang : SERVER_LANG);
...
if (GP_ENABLE && isset($_SESSION['gpack']) && is_string($_SESSION['gpack'])) {
    ...
    if (preg_match('#^gpack/[A-Za-z0-9_\-]+/$#', $__candidate)
        && is_file(dirname(__DIR__, 2) . "/" . $__candidate . "travian.css")) {
